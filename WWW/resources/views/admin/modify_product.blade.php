    ...
    <script>
        function update(id) {
            ...
            if (title.value === "" || /^\s+$/.test(title.value)) {
                ...
                var string="img="+img+"&id="+id+"&class_id="+class_id+"&title="+encodeURIComponent(title.value)+"&content=" + html+"&_token={{csrf_token()}}";
                ajax("{{url("admin/update_product")}}",data=string,function (str) {
                    switch (str) {
                        case "ok":
    ...
    <div class="main-wrap">
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="{{url("admin")}}">首页</a>
                ...
                <span class="crumb-step">&gt;</span><span>编辑产品</span>
            </div>
        </div>
            ...
            <div class="result-content">
                <table class="insert-tab" width="100%">
                    ...
                    <tr>
                        ...
                        <td><input class="common-text required" id="title" size="50" type="text" value="{{$product[0]->title}}"></td>
                    </tr>
                    <tr>
                        <th><i class="require-red">*</i>主图：</th>
                        <td>
                            <input class="common-text required" type="file" name="upload" id="file_upload">
                            <p id="p_img" style="margin-top:10px;"><img style='max-width:200px;max-height:200px;' id="img" src="{{asset("public/upload/product")}}/{{$product[0]->img}}"/></p></td>
                    </tr>
                    @if(count($product_class)>0)
                        <tr>
                            <th><i class="require-red">*</i>分类：</th>
                            <td style="position:absolute;z-index:1;background: #FFFFFF;">
                                <input type="hidden" value="{{$product[0]->class_id}}" id="in_class_id">
                                <div id="sel_div">
                                    <a href="javascript:void(0)" style="padding:7px 10px 7px 5px;border:1px solid #999999;color: #0f0f0f;"
                                       onclick="sel(this)">{{$product[0]->name}}&nbsp;&nbsp;▼</a>
                                    <div>
                    ...
                    <tr>
                        ...
                        <td>
                            <textarea id="editor" name="editor">{{$product[0]->content}}</textarea>
                            <script type="text/javascript">
                    ...
                    <tr>
                        <th></th>
                        <td>
                            <input class="btn btn-primary btn6 mr10" value="提交" type="button" onclick="update('{{$product[0]->id}}')">
                        </td>
                    </tr>
                    </tbody>
                </table>
