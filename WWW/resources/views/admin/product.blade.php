...
<head>
   ...
   input[type=checkbox]{
    ...
    <script>
        function browse(id) {
         ajax("{{url("admin/browse_product")}}","_token={{csrf_token()}}&id="+id,function (str) {
          var obj=eval("("+str+")");
           var html="<div id='browse_div'>" +
              ...
              "</div>";
          G.alert({title:"产品展示",width:600,height:500,content:html,ok:function () {
                  return true;
              }});
    ...
    <script>
        function delone(id) {
            G.alert({content:"确定删除吗？",ok:function () {
                    ajax("{{url("admin/del_product")}}","_token={{csrf_token()}}&id="+id,function (str) {
                        if (str==="ok"){
    ...
    <script>
        function batch_del() {
          ...
          G.alert({content:"确认删除吗?",ok:function () {
                  ...
                  ajax("{{url("admin/del_product")}}",string,function (str) {
                      if (str==="ok"){
                          G.alert({content:"删除成功",ok:function(){window.location=window.location.href;return true;}});
                      }else{
                          G.alert({content:str,ok:function(){return true;}});
    ...
    <div class="result-wrap">
         <div class="result-title">
                <div class="result-list">
                    <a href="{{url("admin/insert_product")}}"><i class="icon-font"></i>新增</a>
            ...
            <div class="result-content">
                <table class="result-tab" width="100%">
                    ...
                    @for($i=0;$i<count($product);$i++)
                        <tr>
                            <td class="tc"><input type="checkbox" class="checkbox" value="{{$product[$i]->id}}"></td>
                            <td>{{$product[$i]->id}}</td>
                            <td>
                                <img style="max-width:100px;max-height:100px;" src="{{asset("public/upload/product")}}/{{$product[$i]->img}}"/>
                                {{$product[$i]->title}}</td>
                            <td>{{($product[$i]->name)}}</td>
                            <td>{{$product[$i]->time}}</td>
                            <td>
                                <a class="link-update" href="javascript:void(0)" onclick="browse('{{$product[$i]->id}}')">
                                    <i style="margin-right:2px;" class="icon-font">&#xe001;</i>浏览 </a>
                                <a class="link-update" href="{{url("admin/modify_product")}}/{{$product[$i]->id}}">
                                    <i style="margin-right:2px;" class="icon-font">&#xe002;</i>编辑 </a>
                                <a class="link-update" href="javascript:void(0)" onclick="delone('{{$product[$i]->id}}')">
                                    <i style="margin-right:2px;" class="icon-font">&#xe019;</i>删除</a>
                            </td>
                        </tr>
                        @endfor
                </table>
                <div class="list-page">{!! $product->links() !!}</div>
            </div>
    </div>
</div>
<!--/main-->
