    ...
    <script>
        function add() {
            ...
            if (title.value === "" || /^\s+$/.test(title.value)) {
                ...
                var string="title="+encodeURIComponent(title.value)+"&content=" + html+"&_token={{csrf_token()}}";
              ajax("{{url("admin/add_news")}}",data=string,function (str) {
                    switch (str) {
                        case "ok":
    ...
    <div class="main-wrap">
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="{{url("admin")}}">首页</a>
                ...
                <span class="crumb-step">&gt;</span><span>新增动态</span>
            </div>
        </div>
